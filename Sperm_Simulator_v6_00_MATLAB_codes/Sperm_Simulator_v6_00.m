...
beat_f = abs(beat_v^.5*(randn(1,numParticles))+beat_m);
ampli = ampli_v^.5*randn(1,numParticles)+ampli_m;
...
radius = radius_v^.5*(randn(1,numParticles))+radius_m;
...
for i = 1: numParticles
    if rand > 0.5
        d_angle_c(i) = var_d^.5*abs(randn(1,1))+ave_d;
    else
        d_angle_c(i) = -(var_d^.5*abs(randn(1,1))+ave_d);
    end
end
...
beat_f = abs(beat_v^.5*(randn)+beat_m);
ampli = ampli_v^.5*randn+ampli_m;

%radius of each particle spin
radius_m = str2double(get(handles.Radius_m,'String'));
radius_v = str2double(get(handles.Radius_v,'String'));
radius = radius_v^.5*(randn)+radius_m;
...
d_angle_c = -(var_d^.5*abs(randn(1,1))+ave_d);
...
function Number_of_videos_Callback(hObject, eventdata, handles)
% hObject    handle to number_of_videos (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of Number_of_videos as text
%        str2double(get(hObject,'String')) returns contents of Number_of_videos as a double


% --- Executes during object creation, after setting all properties.
function Number_of_videos_CreateFcn(hObject, eventdata, handles)
% hObject    handle to number_of_videos (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
